...
def on_click(row, col):
    ...
    # Изменяем цвет текста кнопки в зависимости от игрока
    if current_player == "X":
        buttons[row][col].config(fg="green")  # Игрок 1 (X) - зеленый цвет
    else:
        buttons[row][col].config(fg="red")    # Игрок 2 (O) - красный цвет
